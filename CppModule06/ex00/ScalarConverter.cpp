...
void ScalarConverter::convert(std::string const s)
{
    ...
    else if (s.find('.') != s.npos)
    {
        if (s.find('f') != s.npos)
        {
            ...
            std::cout << "char: " << (std::isprint(static_cast<int>(f)) ? ('\'' + std::string(1, static_cast<int>(f)) + '\'') : ("not displayable")) << std::endl;
        ...
        else
        {
            ...
            std::cout << "char: " << (std::isprint(static_cast<int>(d)) ? ('\'' + std::string(1, static_cast<int>(d)) + '\'') : ("not displayable")) << std::endl;
